...
inline static void __attribute__((always_inline)) check_rbl(char *ip, int *status)
{
  struct hostent *hp;
  struct in_addr ipv4;
...
static char *ngx_http_hostprotect_init_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
  ...
  ngx_conf_merge_uint_value(conf->expire, prev->expire, 60);
  ngx_conf_merge_str_value(conf->purge_ip, prev->purge_ip, "1.1.1.1");
